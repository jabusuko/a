...
local InventoryWatcher = _G.InventoryWatcher or loadstring(game:HttpGet("https://raw.githubusercontent.com/jabusuko/a/refs/heads/main/utils/fishit/inventdetect.lua"))()
...
function AutoFishV3:FindStarterRod()
    ...
    for i, rod in ipairs(fishingRods) do
        ...
        if rodId == 169 or tostring(rodId) == "169" then
            starterRodUUID = rodUUID
            logger:info("Found Ghostfinn Rod! UUID:", starterRodUUID)
            return
        end
    end

    logger:warn("Ghostfinn Rod (ID=169) not found in inventory")
end
...
function AutoFishV3:HandleInitialPhase(rarity)
    ...
    elseif rarity == "Mythic" then
        -- Cancel Mythic but don't affect streak
        logger:info("Mythic detected in INITIAL phase - Canceling (streak unchanged)")
        self:CancelFishing()
    else
...
function AutoFishV3:HandleMythicSecretPhase(rarity)
    if rarity == "Secret" then
        logger:info("Secret detected! Spamming completion to catch it...")
        self:StartCompletionSpam()
    elseif rarity == "Legendary" or rarity == "Rare" or rarity == "Mythic" then
        logger:info(rarity, "detected - canceling to wait for Secret")
        self:CancelFishing()
    else
