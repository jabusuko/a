...
local FeatureManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/jabusuko/a/refs/heads/main/module/f/featuremanager.lua"))()
...
--- AUTO MYTHIC
local autosecretFeature = FeatureManager:Get("AutoSecret")

local autosecret_tgl = VulnBox:AddToggle("autosecrettgl",{
    Text = "Auto Secret",
    Tooltip = "Cancel Fishing until Mythic",
    Default = false,
    Callback = function(Value)
        if autosecretFeature then
            if Value then
                autosecretFeature:Start()
            else
                autosecretFeature:Stop()
            end
        end
    end
})
...
if autosecretFeature then
    autosecretFeature.__controls = {
        toggle = autosecret_tgl
    }
    
    if autosecretFeature.Init and not autosecretFeature.__initialized then
        autosecretFeature:Init()
        autosecretFeature.__initialized = true
    end
end
vulnlabel = VulnBox:AddLabel("Want to know about<br/>this? Join our Discord.")
--- === BACKPACK === ---
--- FAVFISH
